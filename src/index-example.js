...
mapboxgl.accessToken =
  "pk.eyJ1IjoieXVuamllbGkiLCJhIjoiY2lxdmV5MG5rMDAxNmZta3FlNGhyMmpicSJ9.CTEQgAyZGROcpJouZuzJyA";
...
map.on("load", function() {
  ...
  map.on("mousemove", function(e) {
    ...
    if (activeCamera === "hexbin") {
      ...
      if (query.length) {
        ...
        html =
          "<div class='grid grid--gut6 color-white align-center'>" +
          "<div class='col--6'><div class='txt-xl txt-bold custom-ffc300'>" +
          numberComplaints +
          "</div><p class='mx3'>" +
          label +
          " complaints</p></div>" +
          "<div class='col--6'><div class='txt-xl txt-bold custom-ffc300'>" +
          numberBusinesses +
          "</div><p class='mx3'>Restaurants and bars</p></div>" +
          "<div class='col--12 color-gray mt12'>Click to learn more</div></div>";
      }
      ...
      if (queryComplaints.length) {
        ...
        var labelComplaints =
          numberComplaints == 1 ? " complaint" : " complaints";
        var typeComplaints =
          activeType === "total" ? "" : " about " + activeType;
        html +=
          "<div class='grid grid--gut6 color-white align-center'>" +
          "<div class='col--12 px6'><div class='txt-xl txt-bold custom-ffc300'>" +
          numberComplaints +
          "</div><p>" +
          labelComplaints +
          " from this address" +
          typeComplaints +
          "</p></div><div class='col--12 mx12 pl6 grid my12'>";
        ...
        if (activeType === "total") {
          ...
          for (var i = 1; i < typeList.length; i++) {
            html +=
              "<div class='col--4 " +
              typeList[i] +
              " grid pt6 color-gray-light'><div class='icon mx3'></div>" +
              typeCount[i] +
              "</div>";
          }
        }
  ...
  map.on("click", function(e) {
    if (activeCamera === "hexbin") {
      ...
      if (
        query.length &&
        query[0].properties.id === gridActive.features[0].properties.id
      ) {
        // UI changes
        $("#back").show();
    ...
    map.addLayer(
      {
        ...
        filter: ["all", ["!=", activeDDS, 0], ["!=", "businesses", 0]]
      },
  ...
  function setLayers() {
    ...
    if (activeCamera === "hexbin") {
      ...
      map.setPaintProperty("points-businesses", "circle-opacity", 0.2);
  ...
  function setLegend() {
    ...
    $("#style-hexbin .hh1").height(
      Math.max(1, (Math.log(heights[1]) / maxheight) * base)
    );
    $("#style-hexbin .hh2").height(
      Math.max(1, (Math.log(heights[2]) / maxheight) * base)
    );
    $("#style-hexbin .hh3").height(
      Math.max(1, (Math.log(heights[3]) / maxheight) * base)
    );
    $("#style-hexbin .hh4").height(
      Math.max(1, (Math.log(heights[4]) / maxheight) * base)
    );
